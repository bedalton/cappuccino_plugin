    ...
    companion object {
        ...
        fun validateMethodDeclarationParameterClass(selector:ObjJMethodDeclarationSelector, holder: ProblemsHolder) {
            ...
            if (className.text?.startsWith("CG") == true) {
                holder.registerProblem(className, ObjJBundle.message("objective-j.annotator-messages.implementation-annotator.instanceVarClassPNF").format(className.text), ProblemHighlightType.WEAK_WARNING)
                return
            }
            holder.registerProblem(className, ObjJBundle.message("objective-j.annotator-messages.implementation-annotator.instanceVarClassPNF").format(className.text))
        }
