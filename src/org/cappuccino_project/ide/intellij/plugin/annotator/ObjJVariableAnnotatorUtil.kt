...
internal object ObjJVariableAnnotatorUtil {
    ...
    private fun annotateVariableIfOvershadowInstanceVariable(variableName: ObjJVariableName, annotationHolder: AnnotationHolder) {
        ...
        val inheritedClassNames = ObjJInheritanceUtil.getAllInheritedClasses(classDeclarationElement.classNameString, classDeclarationElement.project)
        val annotationLevel = ObjJVariableAnnotatorSettings.OVERSHADOWS_INSTANCE_VARIABLE_SETTING.value
        for (instanceVariableDeclaration in ObjJInstanceVariablesByNameIndex.instance.get(classDeclarationElement.classNameString, project)) {
            ...
            if (instanceVarContainingClass == variableContainingClass) {
                scope = "containing class <" + classDeclarationElement.className + ">"
                break
            }
